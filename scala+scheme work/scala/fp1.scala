...
object fp1 {
  ...
  val factTest : List[Int] = {
    ...
    def factTestRecursive (count:Int) : List[Int] = {
		if (count > 5) 
			Nil
		else
			fact(count)::factTestRecursive (count + 1)
	}
	factTestRecursive(1)
  }
  
/*   val factTestNonRecursive : List[Int] = {
    // TODO: Change "Nil" to the correct answer.
    List[Int](fact(1), fact(2), fact(3), fact(4), fact(5))
  } */
  ...
  def max (xs : List[Int]) : Int = {
    // TODO: Provide definition here.
	def aux (zs:List[Int]) : Int = zs match {
  ...
  /*
  def maxOld (xs : List[Int]) : Int = {
    // TODO: Provide definition here.
	xs match {
		case Nil => throw new NoSuchElementException () 
		case xs => maxHelper (xs, Int.MinValue)
	}
  }
  def maxHelperOld [X] (xs : List[X], max : X) : Int = {
	case (Nil, max) => max
	case (y::ys, max) => {
		if (y >= max)
			maxHelper (ys, y)
  ...
  def max (xs : List[Int]) : Int = {
	...
	xs match {
		case Nil => throw new NoSuchElementException () 
		case _: List[Int] => maxHelper (xs)
	}
  }
  def maxHelper [X] (xs : List[X]) : Int = {
	case Nil => Int.MinValue
	case y:Int :: ys:List[Int] => {
		val tailMax = maxHelper(ys)
		if (y > tailMax)
			y
		else
			tailMax
